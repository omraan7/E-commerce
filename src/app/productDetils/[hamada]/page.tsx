...
export default async function page({ params }: { params: { hamada: string } }) {
  ...
  const { id, brand, images, price, quantity, category, title, priceAfterDiscount, ratingsAverage, ratingsQuantity, description } = ProductDetils

  // ✅ الحل: نضمن إن sold دايمًا number مش undefined
    ...
    <><div className='container mx-auto'>
      ...
      <div className="grid grid-cols-12 gap-3">
        ...
        <div className="col-span-9 p-5 shadow-main-color shadow-lg  ">
          ...
          <div className="my-5">
            {priceAfterDiscount
              ? <><span className="text-black text-2xl font-bold mx-3">{priceAfterDiscount} EGP</span> <span className="text-gray-400 line-through ">{price} EGP</span></>
              : <span className="text-black font-bold mx-3">{price} EGP</span>
            }
          </div>
          <div className=" my-7 pb-6 border-b-2 ">
            {quantity > 0
              ? <span className="text-green-600 p-2 bg-green-100 rounded-2xl w-fit">In Stock</span>
              : <span className="text-red-600 p-2 bg-red-100 rounded-2xl">Out of Stock</span>
            }
          </div>
